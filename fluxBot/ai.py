...
class AI:
	def __init__(self, state_size = 34, action_size = 416, loadAndSave = False, nExpFiles = 100):
	...
	def transformState(self,state,ai_number):
		...
		if ai_number == 2:
			return list(np.array(state).round(decimals=4))
	...
	def makeMove(self,gamestate,controller,ai_number=1):
		...
		if (self.prevState != None) and (self.movesMade > 100):
			...
			self.dqn.remember([prevState],prevAction,reward,[curState],int(done))
